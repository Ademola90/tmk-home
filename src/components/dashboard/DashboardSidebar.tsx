...
const DashboardSidebar = ({ role }: SidebarProps) => {
  ...
  const allLinks = [
    ...
    // Admin Links
    { name: "Admin Dashboard", path: "/dashboard/admin", icon: FiHome },
    {
      name: "Admin Properties",
      path: "/dashboard/admin/properties",
      icon: FiMapPin,
    },
    { name: "Admin Agents", path: "/dashboard/admin/agents", icon: FiUsers },

    // Agent Links
    { name: "Agent Dashboard", path: "/dashboard/agent", icon: FiHome },
    {
      name: "Agent Properties",
      path: "/dashboard/agent/properties",
      icon: FiMapPin,
    },
    { name: "Agent Leads", path: "/dashboard/agent/leads", icon: FiUsers },
  ];

  // For now, show all links regardless of role
  const links = allLinks;

  // Or if you want to keep role-based, use this:
  // const getLinks = () => {
  //   switch (role) {
  //     case "super_admin":
  //       return allLinks; // or superAdminLinks if you want separate
  //     case "admin":
  //       return allLinks.filter(link => link.path.includes('/admin') || link.path.includes('/dashboard/admin'));
  //     case "agent":
  //       return allLinks.filter(link => link.path.includes('/agent') || link.path.includes('/dashboard/agent'));
  //     default:
  //       return allLinks; // Show all for unknown roles
  //   }
  // };
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-6 border-b border-gray-800">
            ...
            <div className="mt-2 text-sm text-gray-400 capitalize">
              {role ? `${role.replace("_", " ")} Panel` : "Dashboard Panel"}
            </div>
          </div>
